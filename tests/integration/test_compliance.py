...
class TestRBICompliance:
    ...
    def test_rbi_disclosure_in_emi_response(self):
        ...
        assert terms_found >= 4, f"Should mention at least 4 financial terms for proper disclosure"
    ...
    def test_prepayment_charge_disclosure(self):
        ...
        charge_terms = ['charge', 'fee', 'penalty', 'prepayment', 'foreclosure', 'cost', 'no charge', 'free']
        has_charge_info = any(term in answer_lower for term in charge_terms)
        
        assert has_charge_info, "Response must disclose prepayment charges or mention no charges"
    ...
    def test_interest_rate_disclosure(self):
        ...
        assert any(term in answer_lower for term in ['interest', 'rate', '%', 'percent', 'annual'])
    ...
    def test_no_misleading_information(self):
        ...
        misleading_terms = ['guaranteed profit', 'risk-free', 'get rich', 'no loss', 'always profitable']
        has_misleading = any(term in answer_lower for term in misleading_terms)
        
        assert not has_misleading, "Response should not contain misleading financial claims"
    ...
    def test_accurate_financial_calculations(self):
        ...
        expected_emi = loan_amount * monthly_rate * pow(1 + monthly_rate, tenure_months) / (pow(1 + monthly_rate, tenure_months) - 1)
...
class TestAuditAndLogging:
    ...
    def test_session_tracking(self):
        ...
        response1 = supervisor.process("What is EMI for customer 1900?", session_id=session_id)
        response2 = supervisor.process("What is the outstanding balance?", session_id=session_id)
...
class TestResponseToneAndCompliance:
    ...
    def test_friendly_professional_tone(self):
        ...
        # Regular query
        response = supervisor.process(
            "What are my loan details for customer 1900?",
            session_id="tone-test-1"
        )
    ...
    def test_error_message_politeness(self):
        ...
        assert len(validation.get('suggestions', [])) > 0, "Should provide suggestions for errors"
        
        # Error response should be constructive
        error_response = calculator._generate_error_response(validation, "test query")
        
        assert 'help' in error_response or 'range' in error_response or 'valid' in error_response
...
class TestDataPrivacyAndSecurity:
    ...
    def test_customer_data_isolation(self):
        ...
        response = supervisor.process(
            "Show loan details for customer 1900",
