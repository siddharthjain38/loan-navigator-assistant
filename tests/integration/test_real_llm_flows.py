...
class TestPolicyGuruRealRAG:
    ...
    def test_real_vector_retrieval(self):
        ...
        try:
            ...
            if len(docs) > 0:
                ...
                print("⚠️ No documents found - vector store may be empty or threshold too high")
        finally:
    ...
    def test_full_rag_response(self):
        ...
        try:
            ...
            response = agent.process("What documents are required for loan application?")
...
class TestWhatIfCalculatorRealCalculations:
    ...
    def test_amortization_schedule_generation(self):
        ...
        response = agent.process({
            "query": "show EMI for different tenures",
            "customer_data": []
        })
...
class TestEndToEndFlows:
    ...
    def test_sql_query_full_flow(self):
        ...
        assert any(word in response.answer.lower() for word in ['loan', '1900', 'customer', 'amount'])
    ...
    def test_calculator_query_full_flow(self):
        ...
        assert any(word in response.answer.lower() for word in ['emi', 'monthly', 'interest'])
    ...
    def test_policy_query_full_flow(self):
        ...
        assert any(word in response.answer.lower() for word in ['prepayment', 'charge', 'policy'])
    ...
    def test_combined_flow_whatif_and_policy(self):
        ...
        assert 'prepay' in answer_lower or 'penalty' in answer_lower or 'policy' in answer_lower
    ...
    def test_unknown_query_fallback(self):
        ...
        response = supervisor.process(
            "xyz abc 123 random nonsense",
            session_id="fallback-test-garbage"
        )
...
class TestFallbackMechanisms:
    ...
    def test_ambiguous_query_clarification(self):
        ...
        if response.metadata.get('requires_clarification'):
            ...
            assert 'clarify' in response.answer.lower() or 'help' in response.answer.lower()
        else:
