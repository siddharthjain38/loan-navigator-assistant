...
class TestPerformanceMetrics:
    ...
    def test_sql_query_performance(self):
        ...
        start_time = time.time()
        response = supervisor.process(
            "Show loan details for customer 1000",
        ...
        assert response.answer is not None
        assert response_time < 25, f"SQL query took {response_time:.2f}s, expected < 25s"
    ...
    def test_policy_retrieval_performance(self):
        ...
        assert response_time < 120, f"Policy retrieval took {response_time:.2f}s, expected < 120s"
...
class TestConversationContext:
    ...
    def test_multi_turn_conversation_context(self):
        ...
        response1 = supervisor.process(
            "Show me loan details for customer 1000",
        ...
        response2 = supervisor.process(
            "What is their outstanding balance?",
            session_id=session_id
        )
        assert response2.answer is not None
        # Should handle the contextual reference
        assert any(word in response2.answer.lower() for word in ['balance', 'outstanding', 'customer'])
    ...
    def test_context_with_clarification(self):
        ...
        assert any(word in answer_lower for word in ['emi', 'amount', 'interest', 'tenure', 'provide', 'specify', 'need'])
...
class TestConcurrencyAndEdgeCases:
    ...
    def test_session_persistence_across_queries(self):
        ...
        response1 = supervisor.process(
            "Show loan for customer 1000",  # SQL
            session_id=session_id
        )
        ...
        response3 = supervisor.process(
            "What are the top-up loan policies?",  # Policy
            session_id=session_id
        )
    ...
    def test_empty_and_whitespace_queries(self):
        ...
        response1 = supervisor.process(
            "",
            session_id="edge-query-1"
        )
        
        # Whitespace only
        response2 = supervisor.process(
            "   ",
            session_id="edge-query-2"
        )
        ...
        for response in [response1, response2]:
            answer_lower = response.answer.lower()
            assert any(word in answer_lower for word in ['help', 'provide', 'ask', 'query', 'question', 'assist', 'specify'])
