...
class TestCriticalCustomerQueries:
    ...
    def test_how_many_emis_left_real_customer(self):
        ...
        response = supervisor.process(
            "How many EMIs are left for customer 1900?",
            session_id="business-test-1"
        )
        ...
        assert any(word in answer_lower for word in ['emi', 'installment', 'payment', 'remaining', 'left', 'month'])
    ...
    def test_topup_eligibility_with_good_history(self):
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['eligible', 'topup', 'top-up', 'qualify', 'additional'])
    ...
    def test_prepayment_impact_on_emi(self):
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['prepay', 'emi', 'change', 'reduce', 'impact'])
    ...
    def test_prepayment_impact_on_tenure(self):
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['tenure', 'month', 'year', 'period', 'reduce', 'shorter'])
    ...
    def test_outstanding_balance_calculation(self):
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['balance', 'outstanding', 'remaining', 'amount', 'loan'])
    ...
    def test_amount_paid_verification(self):
        ...
        # Amount paid query
        response = supervisor.process(
            "How much has customer 1900 paid so far?",
            session_id="business-test-7"
        )
    ...
    def test_next_due_date_query(self):
        ...
        # Next due date query
        response = supervisor.process(
            "When is the next EMI due for customer 1900?",
            session_id="business-test-8"
        )
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['due', 'next', 'date', 'emi', 'payment'])
    ...
    def test_interest_saved_on_prepayment(self):
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['interest', 'save', 'saving', 'reduce', 'prepay'])
    ...
    def test_early_closure_charges(self):
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['charge', 'closure', 'foreclosure', 'early', 'penalty', 'fee'])
...
class TestLoanStatusQueries:
    ...
    def test_loan_status_check(self):
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['status', 'active', 'loan', 'current'])
    ...
    def test_loan_details_summary(self):
        ...
        # Full details query
        response = supervisor.process(
            "Show me all loan details for customer 1900",
            session_id="status-test-2"
        )
    ...
    def test_interest_rate_query(self):
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['interest', 'rate', '%', 'percent'])
