...
class TestMultiLoanScenarios:
    ...
    def test_customer_with_multiple_active_loans(self):
        ...
        response = supervisor.process(
            "Show all loans for customer 1900",
            session_id="multi-loan-test-1"
        )
    ...
    def test_aggregate_emi_across_loans(self):
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['emi', 'total', 'monthly', 'payment'])
    ...
    def test_topup_on_specific_loan(self):
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['eligible', 'topup', 'top-up', 'loan'])
    ...
    def test_outstanding_balance_all_loans(self):
        ...
        answer_lower = response.answer.lower()
        assert any(word in answer_lower for word in ['outstanding', 'balance', 'amount', 'loan'])
...
class TestEdgeCasesAndBoundaries:
    ...
    def test_closed_loan_query(self):
        ...
        response = supervisor.process(
            "Show me closed loans for customer 1900",
        ...
        assert any(word in answer_lower for word in ['loan', 'status', 'closed', 'complete', 'paid', 'active', 'customer'])
    ...
    def test_invalid_customer_id_graceful_error(self):
        ...
        # Should be helpful
        answer_lower = response.answer.lower()
        helpful_terms = ['sorry', 'not found', 'no loan', 'no record', 'customer', 'help', 'verify']
        has_helpful_response = any(term in answer_lower for term in helpful_terms)
        
        assert has_helpful_response or len(response.answer) > 20, "Should provide helpful error message"
    ...
    def test_loan_not_eligible_for_topup(self):
        ...
        # Query for top-up
        response = supervisor.process(
            "Can customer 1902 get a top-up loan?",
        ...
        assert any(word in answer_lower for word in ['eligible', 'topup', 'top-up', 'loan', 'criteria'])
    ...
    def test_prepayment_exceeds_balance_per_loan(self):
        ...
        has_foreclosure_suggestion = any('foreclosure' in sugg.lower() 
                                        for sugg in validation.get('suggestions', []))
    ...
    def test_zero_balance_remaining_query(self):
        ...
        assert any(word in answer_lower for word in ['balance', 'zero', 'paid', 'outstanding', 'closed', '0.00', 'no outstanding', 'fully'])
    ...
    def test_very_large_loan_amount(self):
        ...
        validation = calculator._validate_inputs(loan_amount, interest_rate, tenure_months)
...
class TestComplexScenarios:
    ...
    def test_concurrent_topup_and_prepayment_query(self):
        ...
        has_topup = any(word in answer_lower for word in ['topup', 'top-up', 'additional'])
    ...
    def test_future_scenario_projection(self):
        ...
        response = supervisor.process(
            "What will be my loan status after 6 months?",
            session_id="complex-test-2"
        )
        ...
        assert any(word in answer_lower for word in ['loan', 'month', 'status', 'payment', 'emi', 'help'])
