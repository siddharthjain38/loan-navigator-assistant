...
class LoanDataSeeder:
    ...
    def generate_sample_data(self, num_records: int = 50) -> List[Tuple]:
        ...
        loan_types = [
            ...
            "AL",
            "BL",
        ]  # Home, Personal, Car, Auto, Business
        ...
        for i in range(1, num_records + 1):
            ...
            months_elapsed = min(
                int((datetime.now() - start_date).days / 30), tenure_months
            )
            ...
            topup_eligibility = (
                status == "ACTIVE" and months_elapsed > 12 and random.random() < 0.5
            )
    ...
    def display_sample_records(self, conn: sqlite3.Connection, limit: int = 5) -> None:
        ...
        try:
            ...
            print("-" * 120)
            print(
                f"{'Loan ID':<10} {'Customer':<12} {'Amount':<12} {'Rate':<6} {'EMI':<10} {'Status':<10} {'Top-up':<8}"
            )
            print("-" * 120)
            ...
            for record in records:
                ...
                print(
                    f"{loan_id:<10} {customer_id:<12} {amount:<12,.0f} {rate:<6}% {emi:<10,.0f} {status:<10} {'Yes' if topup else 'No':<8}"
                )
            ...
            cursor = conn.execute(
                "SELECT status, COUNT(*) FROM loan_data GROUP BY status"
            )
    ...
    def seed_database(
        self, num_records: int = 50, force_recreate: bool = False
    ) -> None:
